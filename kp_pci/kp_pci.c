...
* mode ΓÇö either directly (when using the low-level WinDriver API), or
...
* both types of KP_Open callbacks ΓÇö KP_PCI_Open() (standard) and
...
* Kernel PlugIn open function ΓÇö KP_XXX_Open(). When selecting the 32-bit
...
* low-level WD_KernelPlugInCall() function ΓÇö see the WinDriver PCI
...
*     function ΓÇö see the WinDriver PCI Low-Level API Reference) and/or with
...
*     function ΓÇö see the WinDriver PCI Low-Level API
...
*     sample code to the macro COPY_TO_USER_OR_KERNEL ΓÇö This is required
...
* legacy interrupt that has been enabled using a Kernel PlugIn driver ΓÇö
...
*     from WD_IntWait() ΓÇö see the WinDriver PCI Low-Level API Reference).
...
* MSI/MSI-X that has been enabled using a Kernel PlugIn ΓÇö see the
...
* with a Kernel PlugIn handle ΓÇö see WinDriver PCI Low-Level API Reference)
*
* @param [inout] pDrvContext: Driver context data that was set by
* KP_Open() and will also be passed to KP_Close(), KP_IntEnable() and
* KP_Event()
...
* function was called with a Kernel PlugIn handle ΓÇö see the WinDriver PCI
* Low-Level API Reference).
*/
BOOL __cdecl KP_PCI_Event(PVOID pDrvContext, WD_EVENT *wd_event)
{
